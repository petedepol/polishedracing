...
def dim_h(y, x1, x2, label, above=True):  # horizontal dimension line at height y between x1..x2
    off = 1 if above else -1
    return (L((x1, y), (x2, y), 'dimline') + L((x1, y - 12*off), (x1, y + 12*off), 'dimline') + L((x2, y - 12*off), (x2, y + 12*off), 'dimline') + T(((x1 + x2) / 2, y), label, 'dim', dy=-6 if above else 16, anchor='middle'))
...
out = [f'<svg xmlns="http://www.w3.org/2000/svg" class="geometry" viewBox="30 40 {W-60} {H-45}" fill="none" stroke-linecap="round" stroke-linejoin="round" font-family="JetBrains Mono, ui-monospace, monospace" role="img" aria-label="{a.label}: dimensioned side-view geometry drawing of the race bike">',
 '<style>.frame{stroke:#EDEFF2;stroke-width:3}.frame2{stroke:#EDEFF2;stroke-width:2.2}.wheel{stroke:#AEB6BF;stroke-width:1.6}.rim{stroke:#8B93A0;stroke-width:1}.spoke{stroke:#2A2F37;stroke-width:1}.lefty{stroke:#8B93A0;stroke-width:5}.shock{stroke:#3A4049;stroke-width:9}.dimline{stroke:#C6FF2E;stroke-width:1;stroke-dasharray:3 4;opacity:.9}.dim{fill:#C6FF2E;font-size:16px;letter-spacing:.1em}.sub{fill:#8B93A0;font-size:13.5px;letter-spacing:.12em}.axis{stroke:#767E8B;stroke-width:1;stroke-dasharray:2 5}.ghost{stroke:#3A4049;stroke-width:1}</style>']
# wheels + spokes
for c in (RA, FA):
...
out += [L(STt, SP, 'frame2'), L((SP[0] - 130, SP[1] + 22), (SP[0] + 110, SP[1] + 12), 'frame')]
stem_end = (HTt[0] + 65 * math.cos(r(a.ha - 10)), HTt[1] + 65 * math.sin(r(a.ha - 10)) * 0.25 + 12)
out += [L(HTt, stem_end, 'frame2'), L((stem_end[0] - 20, stem_end[1] + 10), (stem_end[0] + 60, stem_end[1] - 6), 'frame')]
...
# dimensions
top = a.stack + 150
out += [dim_h(a.bbdrop - a.wheel - 30, RA[0], FA[0], f'WHEELBASE {wb_calc:.0f}', above=True),
        dim_h(top - 60, 0, a.reach, f'REACH {a.reach:.0f}'),
